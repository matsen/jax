...
// TODO(jevinjiang): Deprecate tpu.rotate and only use tpu.dynamic_rotate. So we
// do not need template for the op type and to explicitly force amount argument
// to dynamic.
template <typename OpTy>
LogicalResult rotate_rule_impl(RewriteContext &ctx, OpTy op, Value amount,
  ...
  // TODO(jevinjiang): make the scratch shape based on config.
  FAILUREOR_ASSIGN_OR_RETURN(
      MemRefType scratch_ref_ty,
      inferMemref(MemRefType::get({9, 8, 128}, vty.getElementType()),
                  ctx.hardware_generation));

  // Some helper functions for math ops.
  auto mlirI32ConstCast = [&](int d) {
    return builder.create<arith::ConstantOp>(
        builder.getIntegerAttr(builder.getI32Type(), d));
  };
  auto mlirIndexConstCast = [&](int d) {
    return builder.create<arith::ConstantOp>(
  ...
  auto modI = [&](const Value &v, unsigned d) -> Value {
    if (auto cst = getIntConst(v, /*silent=*/true); succeeded(cst)) {
      return mlirI32ConstCast(cst.value() % d);
    }
    return builder.create<arith::RemUIOp>(v, mlirI32ConstCast(d));
  };
  auto divI = [&](const Value &v, unsigned d) -> Value {
    if (auto cst = getIntConst(v, /*silent=*/true); succeeded(cst)) {
      return mlirI32ConstCast(cst.value() / d);
    }
    return builder.create<arith::DivUIOp>(v, mlirI32ConstCast(d));
  };
  auto addI = [&](const Value &v, unsigned d) -> Value {
    if (auto cst = getIntConst(v, /*silent=*/true); succeeded(cst)) {
      return mlirI32ConstCast(cst.value() + d);
    }
    return builder.create<arith::AddIOp>(v, mlirI32ConstCast(d));
  };

  auto getVmaskByPaddingEnd = [&](Value padding, int dim, int stride = 0) {
    CHECK(dim == 0 || dim == 1);
    Value padding_vreg = builder.create<vector::BroadcastOp>(i32_vreg, padding);
    if (stride > 0) {
  ...
  rotate = [&](const xla::Array<Value> &vregs, Value shift, int axis,
    ...
    CHECK((tiling_dim == 0 && stride == 0) ||
          (tiling_dim == 1 && stride >= 0) || (tiling_dim < 0 && stride == 0));
    ...
    if (auto shift_cst = getIntConst(shift, /*silent=*/true);
      ...
      if (tiling_dim >= 0) {
        shift = mlirI32ConstCast(static_shift % ctx.target_shape[tiling_dim]);
    ...
    // TODO(jevinjiang): change the 9 based on the internal scratch config.
    if (bound <= 9) {
      // Use static store + dynamic load to implement dynamic shift.
      auto scratch_ref =
          builder.create<tpu::GetInternalScratchOp>(scratch_ref_ty);
      auto cst_0 = mlirIndexConstCast(0);
      ...
      chunks[0].Each([&](absl::Span<const int64_t> idxs, Value *v) {
        ...
        for (int i = 0; i < bound; ++i) {
          scratch_indices[0] = mlirIndexConstCast(i);
          builder.create<tpu::StoreOp>(chunks[i](idxs), scratch_ref,
        ...
        for (int i = 0; i < bound; ++i) {
          scratch_indices[0] = builder.create<arith::IndexCastOp>(
              ...
              modI(builder.create<arith::SubIOp>(mlirI32ConstCast(bound + i),
                                                 vreg_shift),
                   bound));
          chunks[i](idxs) = builder.create<tpu::LoadOp>(
              v->getType(), scratch_ref, scratch_indices, sublane_mask_attr,
              /*sublane_stride=*/nullptr);
        }
      });
      result = concatenate(chunks, axis);
    } else {
      // Convert dynamic shift to log(bound) static ops.
      int roll_by = 1;
      Value cst_1 = mlirI32ConstCast(1);
      while (bound > 0) {
        auto new_result =
            rotate(result,
                   mlirI32ConstCast(tiling_dim >= 0
                                        ? roll_by * ctx.target_shape[tiling_dim]
                                        : roll_by),
                   axis, /*stride=*/0);
        auto mask = builder.create<arith::CmpIOp>(
            ...
            builder.create<arith::ConstantOp>(DenseElementsAttr::get(
                i32_vreg, builder.getI32IntegerAttr(0))));
...
// TODO(jevinjiang): deprecate the static rotate.
