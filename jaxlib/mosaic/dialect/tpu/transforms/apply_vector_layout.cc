...
// Get the address of preallocated internal scratch space with requested shape.
//
// Arguments:
...
FailureOr<Value> getInternalScratch(RewriteContext &ctx, OpBuilder &builder,
  ...
  if (*shape.end() % ctx.target_shape[1] != 0) {
    return failure();
  }
  int sublane_count = std::accumulate(shape.begin(), shape.end() - 1, 1,
                                      std::multiplies<int>());
